...
export function QuestionDrilldown({ detail, mutable, adjustmentOpen, onAdjust, onOpenGradebook, adjustmentForm }: { detail: any; mutable: boolean; adjustmentOpen: boolean; onAdjust: () => void; onOpenGradebook?: () => void; adjustmentForm?: React.ReactNode }) {
  ...
  const adjustable = mutable && detail.status === "FINALIZED" && detail.systemScore !== null && Number(detail.v5BaseScore) > 0;
...
function LegacyMissionScorePanel({ mission, mutable, adjustmentOpen, onAdjust, onOpenGradebook, adjustmentForm }: { mission: any; mutable: boolean; adjustmentOpen: boolean; onAdjust: () => void; onOpenGradebook?: () => void; adjustmentForm?: React.ReactNode }) {
  const override = currentOverride(mission.adjustmentStream);
  const rubric = mission.rubricBreakdown;
  const rounds = mission.roundBreakdown;
  return <section className="card scoreSection"><div className="row between"><div><p className="eyebrow">Mission score</p><h2>{mission.mission.stableId}</h2></div><div className="row">{onOpenGradebook && <button type="button" className="button secondary" onClick={onOpenGradebook}>Open in Gradebook</button>}{mutable && mission.systemScore !== null && !adjustmentOpen && <button type="button" className="button secondary" onClick={onAdjust}>Adjust Mission</button>}</div></div>{rubric && <><h3>Authoritative strategy rubric</h3><div className="scoreFacts scoreFactsWide"><span>Compliance</span><strong>{score(rubric.compliance)} / 40.00</strong><span>Cost Logic</span><strong>{score(rubric.costLogic)} / 30.00</strong><span>Position</span><strong>{score(rubric.position)} / 20.00</strong><span>Reasoning</span><strong>{score(rubric.reasoning)} / 10.00</strong></div><div className="notice">Question Overrides annotate question evidence but do not rewrite the Mission 5 strategy rubric. A Mission Override supersedes the system Mission score.</div></>}{rounds && <><h3>Mission 6 round breakdown</h3><div className="scoreFacts scoreFactsWide"><span>Round 1 Policy Shock</span><strong>{score(rounds.round1)} / 50.00</strong><span>Round 2 Technology Shock</span><strong>{score(rounds.round2)} / 25.00</strong><span>Round 3 Integrated</span><strong>{score(rounds.round3)} / 25.00</strong></div></>}<div className="scoreFacts scoreFactsWide"><span>System Mission Score</span><strong>{score(mission.systemScore)} / 100.00</strong><span>{rubric ? "Rubric score after Question Overrides" : "Question-level adjusted calculation"}</span><strong>{score(mission.questionAdjustedScore)}</strong><span>Current Mission Override</span><strong>{score(override)}</strong><span>Effective Mission Score</span><strong>{score(mission.effectiveScore ?? effectiveMissionScore(mission))}</strong></div><h3>Mission adjustment history</h3><AdjustmentHistory stream={mission.adjustmentStream} />{adjustmentOpen && adjustmentForm}</section>;
}
...
export function MissionScorePanel({ mission, mutable, adjustmentOpen, onAdjust, onOpenGradebook, adjustmentForm }: { mission: any; mutable: boolean; adjustmentOpen: boolean; onAdjust: () => void; onOpenGradebook?: () => void; adjustmentForm?: React.ReactNode }) {
  const override = currentOverride(mission.adjustmentStream);
  const breakdown = (mission.scoreBreakdown ?? (mission.rubricBreakdown ? { missionStableId: "M5", components: [{ stableId: "COMPLIANCE", earned: mission.rubricBreakdown.compliance, maximum: 40, status: "ACTIVE" }, { stableId: "COST_LOGIC", earned: mission.rubricBreakdown.costLogic, maximum: 30, status: "ACTIVE" }, { stableId: "POSITION", earned: mission.rubricBreakdown.position, maximum: 20, status: "ACTIVE" }, { stableId: "REASONING", earned: mission.rubricBreakdown.reasoning, maximum: 10, status: "ACTIVE" }], rawActiveTotal: mission.rubricBreakdown.total, rawActiveMaximum: 100, normalizedTotal: mission.rubricBreakdown.total, normalizedMaximum: 100, normalizationApplied: false } : null)) as MissionScoreBreakdownDto | null;
  return <section className="card scoreSection">
    <div className="row between"><div><p className="eyebrow">Mission score</p><h2>{mission.mission.stableId}</h2></div><div className="row">{onOpenGradebook && <button type="button" className="button secondary" onClick={onOpenGradebook}>Open in Gradebook</button>}{mutable && mission.systemScore !== null && !adjustmentOpen && <button type="button" className="button secondary" onClick={onAdjust}>Adjust Mission</button>}</div></div>
    {breakdown && <div className="missionBreakdown"><h3>Mission Score Breakdown</h3>{breakdown.components.map((component) => <div className="breakdownRow" key={component.stableId}><span>{missionScoreComponentLabel(component.stableId)}{component.status === "INACTIVE_UNMAPPED" && <small>Inactive — not earnable</small>}</span><strong>{component.status === "ACTIVE" && component.earned !== null ? `${score(component.earned)} / ${score(component.maximum)}` : `— / ${score(component.maximum)}`}</strong></div>)}{breakdown.normalizationApplied && <p className="muted">Active raw structure: {score(breakdown.rawActiveTotal)} / {score(breakdown.rawActiveMaximum)}, normalized to the Mission total.</p>}<div className="breakdownRow breakdownTotal"><span>Total</span><strong>{score(breakdown.normalizedTotal)} / 100.00</strong></div></div>}
    {mission.mission.stableId === "M5" && <><h3>Authoritative strategy rubric</h3><div className="notice">Question Overrides annotate question evidence but do not rewrite the Mission 5 strategy rubric. A Mission Override supersedes the system Mission score.</div></>}
    <div className="scoreFacts scoreFactsWide"><span>System Mission Score</span><strong>{score(mission.systemScore)} / 100.00</strong><span>{mission.mission.stableId === "M5" ? "Rubric score after Question Overrides" : "Question-level adjusted calculation"}</span><strong>{score(mission.questionAdjustedScore)}</strong><span>Current Mission Override</span><strong>{score(override)}</strong><span>Effective Mission Score</span><strong>{score(mission.effectiveScore ?? effectiveMissionScore(mission))}</strong></div>
    <h3>Mission adjustment history</h3><AdjustmentHistory stream={mission.adjustmentStream} />{adjustmentOpen && adjustmentForm}
  </section>;
}
...
export function FinalScorePanel({ participant, attempt, mutable, adjustmentOpen, onAdjust, adjustmentForm }: { participant: any; attempt: any; mutable: boolean; adjustmentOpen: boolean; onAdjust: () => void; adjustmentForm?: React.ReactNode }) {
  ...
  const calculated = attempt.calculatedFinalScore ?? attempt.systemTotalScore;
  const hasAuthoritativeTotal = calculated !== null && calculated !== undefined;
  return <section className="card scoreSection"><div className="row between"><div><p className="eyebrow">Final Carbon Market IQ</p><h2>{participant.student.name}</h2></div>{mutable && hasAuthoritativeTotal && !adjustmentOpen && <button type="button" className="button secondary" onClick={onAdjust}>Adjust Final Total</button>}</div><div className="scoreFacts scoreFactsWide"><span>Calculated/System Final Total</span><strong>{score(calculated)}{hasAuthoritativeTotal ? " / 100.00" : ""}</strong><span>Current Final Override</span><strong>{score(override)}</strong><span>Effective Final Total</span><strong>{score(attempt.effectiveScore ?? effectiveFinalScore(attempt))}</strong></div>{!hasAuthoritativeTotal && <div className="notice">Final total is not available until all six authoritative Mission scores exist. No provisional score is fabricated.</div>}<h3>Final Total adjustment history</h3><AdjustmentHistory stream={attempt.adjustmentStream} />{adjustmentOpen && adjustmentForm}</section>;
}

export default function ScoreWorkspace({ selected, request, gradebook, initialSelection = null, onOpenGradebook, registerRefresh }: { selected: WorkshopRef | null; request: Request; gradebook: boolean; initialSelection?: GradebookSelection | null; onOpenGradebook?: (selection: GradebookSelection) => void; registerRefresh?: (task:()=>Promise<void>)=>()=>void }) {
  ...
  if (loadPresentation.showLoading && !hasLoaded) return <><p className="eyebrow">{gradebook ? "Gradebook" : "Students"}</p><h1>{gradebook ? "Scores, evidence and audit" : "Workshop roster"}</h1><div className="card muted">Loading student attempts…</div></>;
  if (loadPresentation.showError && !hasLoaded) return <><p className="eyebrow">{gradebook ? "Gradebook" : "Students"}</p><h1>{gradebook ? "Scores, evidence and audit" : "Workshop roster"}</h1><div className="error">{loadError}</div></>;
