...
export const answerRuleSchema = z.discriminatedUnion("mode", [
  z.object({ mode: z.literal("EXACT"), answer: z.union([z.string(), z.array(z.string()), z.record(z.string())]) }),
  z.object({ mode: z.literal("MULTI_EXACT"), answer: z.array(z.string()) }),
  z.object({ mode: z.literal("NUMERIC"), answer: z.number(), tolerance: z.number().nonnegative() }),
  z.object({ mode: z.literal("STRATEGY"), rubricRef: z.string(), rubric: z.record(z.number()) })
]);
