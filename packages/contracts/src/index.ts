...
export const PASSWORD_REQUIREMENT = `Password must be at least ${PASSWORD_MIN_LENGTH} characters.`;
export const usernameSchema = z.string().trim().min(3).max(80);
export const passwordSchema = z.string().min(PASSWORD_MIN_LENGTH, PASSWORD_REQUIREMENT).max(PASSWORD_MAX_LENGTH);
export const loginSchema = z.object({ username: usernameSchema, password: passwordSchema });
export const studentRegistrationSchema = loginSchema.strict();
export const managedTeacherRoleSchema = z.enum(["INSTRUCTOR", "VIEWER"]);
export const createTeacherAccountSchema = z.object({ username: usernameSchema, displayName: z.string().trim().min(1).max(150), role: managedTeacherRoleSchema, password: passwordSchema }).strict();
export const changeTeacherRoleSchema = z.object({ role: managedTeacherRoleSchema }).strict();
export const changeTeacherStatusSchema = z.object({ status: z.enum(["ACTIVE", "DISABLED"]) }).strict();
export const resetTeacherPasswordSchema = z.object({ password: passwordSchema }).strict();
export const changeOwnPasswordSchema = z.object({ currentPassword: passwordSchema, newPassword: passwordSchema }).strict();
